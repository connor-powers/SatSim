...
TEST(AttitudeTests, PassivePitchTest1) {
  ...
  EXPECT_TRUE(abs(evolved_pitch - initial_pitch) <= abs(after_loop_time - orbital_period))
      << "Pitch didn't progress 2*pi radians over one orbit as expected. Difference: "
      << evolved_pitch - initial_pitch << ", whereas the difference between the initial "
      " and after-loop true anomalies was " << after_loop_time - orbital_period
      << "\n";
}

...
double PhasedArrayGroundStation::distance_to_satellite(
    Satellite input_satellite) {
  // Computes Euclidean distance to a satellite
  std::array<double, 3> satellite_position_ECI =
      input_satellite.get_ECI_position();
  ...
  Vector3d difference_vec =
      (satellite_position_vec - groundstation_position_ECI);
  return difference_vec.norm();
}

double PhasedArrayGroundStation::angle_to_satellite_from_normal(
    Satellite input_satellite) {
  ...
  // but I don't think you need to use that topocentric-horizontal coordinate
  // system to just get angle between antenna plane normal and station-satellite
  // vector
  double current_time = input_satellite.get_instantaneous_time();
  Vector3d groundstation_position_ECI = get_ECI_position(current_time);
  ...
  Vector3d station_to_satellite_vec =
      satellite_position_vec - groundstation_position_ECI;
  ...
  double angle_radians =
      acos(station_to_satellite_vec.dot(antenna_plane_normal_direction));
  // Make sure returned angle is in degrees
  return (angle_radians * (180 / M_PI));
}

int PhasedArrayGroundStation::num_sats_connected_at_this_time(
    double input_time) {
  // Goal: check the number of satellites whose time ranges of being linked with
  // this ground station include the current time
  ...
  for (auto key_val_pair : linked_sats_map_) {
    ...
    std::vector<std::pair<double, double>> satellite_comms_range_list =
        key_val_pair.second;
    // For a given satellite
    // I just need to identify whether there exists a single range that
    // encompasses the input time I can do this by just finding the last range
    // where the t_start <= input_time and see if t_end >= input_time
    if (satellite_comms_range_list.size() == 1) {
      if ((satellite_comms_range_list.at(0).first <= input_time) &&
          (satellite_comms_range_list.at(0).second >= input_time)) {
        connected_sats += 1;
        continue;
      }
    }
    ...
    while (right_ind >= 0) {
      std::pair<double, double> right_range =
          satellite_comms_range_list.at(right_ind);
...
void PhasedArrayGroundStation::update_linked_sats_map(
    const size_t satellite_index, const double connection_time,
    const double previous_time) {
  // If you're just extending an existing range, update the t_end of existing
  // pair, otherwise create a new pair with the current time as the t_start
  if (linked_sats_map_.count(satellite_index) == 0) {
    std::pair<double, double> connection_time_range = {connection_time,
                                                       connection_time};
    ...
    std::vector<std::pair<double, double>> existing_connection_time_ranges =
        linked_sats_map_[satellite_index];
    std::pair<double, double> most_recent_range =
        existing_connection_time_ranges.at(
            existing_connection_time_ranges.size() - 1);
    if (most_recent_range.second == previous_time) {
      // Just extending existing range
      most_recent_range.second = connection_time;
      existing_connection_time_ranges.at(
          existing_connection_time_ranges.size() - 1) = most_recent_range;
      linked_sats_map_[satellite_index] = existing_connection_time_ranges;
    } else {
      // New connection range
      std::vector<std::pair<double, double>> existing_connection_time_ranges =
          linked_sats_map_[satellite_index];
      std::pair<double, double> new_connection_time_range = {connection_time,
                                                             connection_time};

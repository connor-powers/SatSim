...
int main() {
  ...
  PhasedArrayGroundStation example_ground_station_1(
      ...
      num_beams);
  std::vector<Satellite> long_sat_vec = {test_sat_1,  test_sat_2,  test_sat_3,
                                         test_sat_10, test_sat_11, test_sat_12,
                                         test_sat_13};
  std::string coverage_file_name = "Ground station connectivity with " +
                                   std::to_string(num_beams) + " beams";
  sim_and_plot_gs_connectivity_gnuplot(example_ground_station_1, long_sat_vec,
                                       sim_parameters, coverage_file_name);

  num_beams = 2;
  PhasedArrayGroundStation example_ground_station_2(
      gs_latitude, gs_longitude, gs_altitude, max_beam_angle_from_normal,
      num_beams);
  coverage_file_name = "Ground station connectivity with " +
                       std::to_string(num_beams) + " beams";
  sim_and_plot_gs_connectivity_gnuplot(example_ground_station_2, long_sat_vec,
                                       sim_parameters, coverage_file_name);

  // Now demonstrating low-thrust orbital transfer
  Satellite orbit_raising_demo_sat(
      "../example_satellite_input_files/"
      "circular_orbit_raising_test_input.json");
  Satellite orbit_lowering_demo_sat(
      "../example_satellite_input_files/"
      "circular_orbit_lowering_test_input.json");
  double raising_final_orbit_semimajor_axis = 20000;  // km
  thrust_magnitude = 1;                               // N
  double transfer_initiation_time = 0;                // s
  int error_code_raising = add_lowthrust_orbit_transfer(
      orbit_raising_demo_sat, raising_final_orbit_semimajor_axis,
      thrust_magnitude, transfer_initiation_time);

  double lowering_final_orbit_semimajor_axis = 10000;  // km
  int error_code_lowering = add_lowthrust_orbit_transfer(
      orbit_lowering_demo_sat, lowering_final_orbit_semimajor_axis,
      thrust_magnitude, transfer_initiation_time);
  std::vector<Satellite> orbit_transfer_demo_vec = {orbit_raising_demo_sat,
                                                    orbit_lowering_demo_sat};
  sim_parameters.initial_timestep_guess = 2;
  ...
  // sim_and_plot_orbital_elem_gnuplot(orbit_transfer_demo_vec, sim_parameters,
  // "Semimajor Axis", file_name);
  ...
  std::vector<Satellite> arg_of_periapsis_transfer_vec = {
